...
// console.log('FCFCommonapprovalhandler: options:', options);
    ...
    def.then(function(model) {
        ...
        if (newmodel) {
			...
			var oldValues = {};
			pops.forEach(function(key){
				oldValues[key] = newmodel[key];
			})

			// AD.log('... found it');
            // set status to 'approved'
            newmodel.status = 'denied';
            
            async.waterfall([
                function (next) {
                    // Save the activity photo
                    newmodel.save()
        			.then(function(updatedModel) {
                        ...
                        // do another lookup:
                        for (var v in oldValues) {
                            newmodel[v] = oldValues[v];
                        }

                        // Add menu info
                        newmodel.menu = options.menu;
                        next();
                ...
                // Get guid from uploadedBy ID
        		function (next) {
                    // console.log("this is next", newmodel);
                    GUID2Person.findOne({person:newmodel.uploadedBy.IDPerson})
                    .then(function(data){
                        
                        // console.log("guid2person:", data);
                        next(null, data.guid);
                ...
                // Get site_user's email preference from guid
                function (guid, next) {
                    // console.log("finding the person now:", guid);
                    SiteUser.findOne({ guid: guid })
                    .then(function (result) {
                        // console.log("sendEmailNotification:", result);
                        if (result.sendEmailNotifications == 1) {
                            // console.log("continue");
                            next();
                        } else {
                            next("User does not want emails");
                        }
                    })
                    .catch(function(err){
                        AD.log(err);
                ...
                // if the user wants emails sent look up their person ID
                function (next) {
                    // console.log("looking for person who uploaded the photo");
                    FCFPerson.findOne({IDPerson:newmodel.uploadedBy.IDPerson})
                ...
                function (uploader, next) {
                    ...
                    .then(function(details){
                        ...
                        // console.log("cmdetails results:", details);
                    ...
                    .catch(function(err){
                        ADCore.error.log('fcf_core: Error looking up FCFCMDetails', {
                            error:err,
                            cond:comments.deniedBy.IDPerson
                        });
                        // next(err);
                    })
                },
                // find the uploaders email address
                function (uploader, next) {
...
if (comments) {
                    ...
                    <a href="https://adroit.appdevdesigns.net" class="button" style="text-decoration: none;background: #f1592a;color: white;padding: 15px;font-size: 1.25em;margin: 20px 0;display: block;border-radius: 5px;text-align: center;">Fix my activity photo</a><br/>
                    <small style="color: white;">This email was sent from an unmonitored mailbox. Please do not email this address directly.</small><br/>
                    <br/>
                    <br/>
                    </div>
                        ...
                        (response) => {
                            ...
                            transport.sendMail(mailOptions, function(error, info){
                                if (error) {
                                    ...
                                    next(null, 'Email sent: ' + info.response);
                                }
                            });
                        }
                    )
