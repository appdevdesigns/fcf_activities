...
////        we should not have to do this here.
    ...
    mobileMyProjects:function(req, res) {
        ...
        ProjectMembers(guid, req.query, (err, data)=>{
    ...
    mobileMyProjectMembers: function(req, res) {
        ...
        ProjectMembers(guid, req.query, (err, allProjects)=>{

            // now take this data, and repackage with all members seperately:

            
            var hashAllMembers = { /* memberID : {member} */ };
            var teamMemberIDs = [];
            var allMembers = [];

            async.series([

                // gather all Project Member's 
                // and team members:
                (next)=>{
                    allProjects.forEach((project)=>{
                        ...
                        project.teams.forEach((team)=>{
                            teamMemberIDs = teamMemberIDs.concat(team.memberIDs);
                        })
                    })

                    // only return those memberIDs that don't already exist in hashAllMembers:
                    teamMemberIDs = teamMemberIDs.filter((id)=>{ return !(hashAllMembers[id])});

                    next();
                },

                // gather all members from each team:
                (next) => {

                    FCFPerson.find({IDPerson:teamMemberIDs})
                ...
                allMembers = Object.keys(hashAllMembers).map((key)=>{
                    return hashAllMembers[key];
                })
...
/**
 * ProjectMembers()
 * process the projects + teams & members for the given user guid.
 * @param {uuid} guid the uuid of the current user
 * @param {fn} cb  node style callback with the compiled data:
 *              data: [ {project}, {project}, ... ]
 */
function ProjectMembers(guid, query, cb) {
